...
module.exports = (client) => {
  client.on("messageCreate", async (message) => {
    ...
    if (command === "!kothstatus") {
      if (activeKothGames.has(channelId)) {
        ...
        return message.channel.send(
          "No active King of the Hill game in this channel. Start one with `!koth [amount]`!"
        );
      }
    }
  });
  ...
  async function startNewKothGame(message, userId, amount, channelId) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "⏰ Time Remaining",
          value: `<t:${Math.floor(gameData.endTime / 1000)}:R>`,
          inline: true,
        },
        {
          name: "🎯 How to Challenge",
          value: `Use \`!koth [amount]\` to challenge the King!`,
          inline: false,
        },
        {
          name: "📊 Challenge Formula",
          value: `50% chance at equal bets, up to 95% max`,
          inline: false,
        }
      )
  ...
  async function handleChallenge(message, userId, challengeAmount, channelId) {
    ...
    if (userId === gameData.kingId) {
      return message.channel.send(
        "❌ You can't challenge yourself! You're already the King!"
      );
    }
    ...
    if (Date.now() > gameData.endTime) {
      ...
      return message.channel.send(
        "⏰ The previous game has ended! Start a new one with `!koth [amount]`"
      );
    }
    ...
    if (success) {
      ...
      resultEmbed = new EmbedBuilder()
        ...
        .setColor("#00ff00")
        .setDescription(
          `🎉 **${message.author.username}** has overthrown the King!`
        )
        .setImage("attachment://koth-overthrow.png")
        .addFields(
          {
            name: "⚔️ Challenge Result",
            value: `**Success!** (${(chance * 100).toFixed(1)}% chance)`,
            inline: true,
          },
          {
            name: "🎲 Roll",
            value: `${(roll * 100).toFixed(1)}% (needed <${(chance * 100).toFixed(1)}%)`,
            inline: true,
          },
      ...
      resultEmbed = new EmbedBuilder()
        ...
        .setColor("#ff0000")
        .setDescription(
          `💀 **${message.author.username}**'s challenge has failed!`
        )
        .setImage("attachment://koth-failed.png")
        .addFields(
          {
            name: "⚔️ Challenge Result",
            value: `**Failed!** (${(chance * 100).toFixed(1)}% chance)`,
            inline: true,
          },
          {
            name: "🎲 Roll",
  ...
  async function showGameStatus(message, channelId) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "🎯 Challenge Info",
          value: `Equal bet = 50% chance, higher = better odds!`,
          inline: true,
        },
        {
          name: "📊 Win Formula",
          value: `50% at equal bets, max 95% chance`,
          inline: true,
        }
      )
  ...
  async function endKothGame(channelId, client) {
    ...
    try {
      ...
      if (channel) {
        const finalCanvas = await createKothEndVisual(
          gameData,
          winnings,
          houseCut
        );
        const attachment = new AttachmentBuilder(finalCanvas.toBuffer(), {
          name: "koth-victory.png",
        });
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor("#ffd700")
          .setDescription(
            `🎉 **${gameData.kingName}** has successfully defended the throne!`
          )
          ...
          .addFields(
            ...
            {
              name: "⏰ Reign Duration",
              value: `${Math.floor((Date.now() - gameData.startTime) / 60000)} minutes`,
              inline: true,
            },
            { name: "👑 Royal Status", value: "Unchallenged!", inline: true }
          )
          .setFooter({
            text: "The King claims victory! Start a new game with !koth [amount]",
          })
          .setTimestamp();
  ...
  async function createKothVisual(gameData, guild) {
    ...
    ctx.font = "18px Arial";
    ctx.fillText(
      `Time: ${minutesLeft}:${secondsLeft.toString().padStart(2, "0")}`,
      300,
      30
    );
    ...
    ctx.font = "16px Arial";
    ctx.fillText(
      `King's Power: 🍯${gameData.kingAmount.toLocaleString()}`,
      300,
      380
    );
