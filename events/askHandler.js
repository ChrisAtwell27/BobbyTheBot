...
module.exports = (client) => {
    console.log('🎱 Magic 8-Ball Handler (Gemini AI) initialized');
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (command === '!ask' || command === '!8ball' || command === '!magic8ball') {
            ...
            if (!question || question.trim().length === 0) {
                return message.reply('🎱 Please ask me a question! Example: `!ask Will I have a good day?`');
            }
            ...
            if (!GEMINI_API_KEY) {
                return message.reply('❌ The magic 8-ball oracle is not properly configured. Please contact an administrator.');
            }

            try {
                console.log(`🎱 ${message.author.tag} asked: "${question}"`);
                ...
                return message.reply('❌ The mystical forces are currently disrupted. Please try again later!');
            }
        }
        ...
        if (command === '!8balltest') {
            ...
            if (!message.member.permissions.has('Administrator')) {
                return message.reply("❌ You need Administrator permissions to test the API connection.");
            }

            if (!GEMINI_API_KEY) {
                return message.reply('❌ No API key configured. Please set GEMINI_API_KEY environment variable or add geminiApiKey to config.');
            }

            try {
                const testResponse = await getGeminiResponse("Is this API working?");
                return message.reply(`✅ API test successful! Response: "${testResponse}"`);
            } catch (error) {
                return message.reply(`❌ API test failed: ${error.message}`);
            }
        }
    });
};