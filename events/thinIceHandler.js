...
try {
  ...
  const allWords = badWordsContent
    .split(",")
    .map((word) => word.trim().replace(/['"]+/g, ""))
    .filter((word) => word.length > 0);

  // Filter out excluded words
  badWords = allWords.filter(
    (word) => !excludedWords.includes(word.toLowerCase())
  );

  console.log(
    `❄️ Thin Ice Handler: Loaded ${badWords.length} bad words (${allWords.length - badWords.length} excluded)`
  );
} catch (error) {
...
module.exports = async (client) => {
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (messageContent.startsWith("!reset thinice")) {
      ...
      if (!mentionedUser) {
        return message.reply(
          "❌ Please mention a valid user to reset Thin Ice."
        );
      }
      ...
      if (member.roles.cache.has(thinIceRoleId)) {
        ...
        console.log(
          `❄️ Thin Ice reset for ${member.user.tag} by ${message.author.tag}`
        );
      } else {
    ...
    if (
      messageContent === "!thinice" ||
      messageContent.startsWith("!thinice ")
    ) {
      const mentionedUser = message.mentions.users.first();
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "❄️ Status",
            value: hasThinIce ? "On Thin Ice" : "Clear",
            inline: true,
          },
          {
            name: "⏭️ Next Penalty",
            value: getNextPenalty(warnings),
            inline: true,
          }
        )
        .setTimestamp();
    ...
    if (foundBadWord) {
      console.log(
        `❄️ Detected bad word "${foundBadWord}" in message from ${message.author.tag}: "${message.content}"`
      );
        ...
        if (warnings === 0) {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              {
                name: "⚠️ Offense",
                value: "Inappropriate language directed at Bobby",
                inline: false,
              },
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              {
                name: "⚠️ Penalty",
                value: "Timeout for 5 minutes",
                inline: false,
              },
              { name: "🔢 Warning Level", value: "3/3", inline: true },
              {
                name: "⏭️ Next Penalty",
                value: "10 minute timeout",
                inline: true,
              }
            )
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields(
              {
                name: "⚠️ Penalty",
                value: `Timeout for ${Math.floor(timeoutDuration / 60000)} minutes`,
                inline: false,
              },
              {
                name: "🔢 Warning Level",
                value: `${warnings + 1} (Critical)`,
                inline: true,
              },
              {
                name: "📢 Note",
                value: "Contact a moderator to appeal",
                inline: true,
              }
            )
            .setFooter({
              text: "Continued violations may result in escalation",
            })
            .setTimestamp();
          ...
          await member.timeout(
            timeoutDuration,
            `Broke thin ice (${warnings + 1}th offense)`
          );
          updateUserWarnings(member.id, thinIceFilePath, warnings + 1);
        }

        console.log(
          `❄️ Thin Ice: ${member.user.tag} warning level ${warnings + 1}`
        );
      } catch (error) {
    ...
    if (hadThinIceRole && !hasThinIceRoleNow) {
      try {
        ...
        if (warnings > 0) {
          ...
          console.log(
            `Reassigned Thin Ice role to ${newMember.user.tag} because they have ${warnings} warnings.`
          );
        }
      } catch (error) {
  ...
  function updateUserWarnings(userId, filePath, warnings) {
    try {
      let data = fs.readFileSync(filePath, "utf-8");
      const lines = data.split("\n").filter((line) => line.trim() !== "");
      const userRecordIndex = lines.findIndex((line) =>
