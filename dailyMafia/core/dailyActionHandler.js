...
const { processNightActions: processSharedNightActions } = require('../../mafia/game/mafiaActions');
  ...
  try {
    ...
    for (const player of alivePlayers) {
      ...
      try {
        ...
        const targets = alivePlayers.filter(p => p.playerId !== player.playerId);
        const prompt = buildActionPrompt(player, role, targets, game);

        await user.send(prompt);

        console.log(`[Daily Mafia ${gameId}] Sent night action prompt to ${player.displayName}`);
      } catch (error) {
        console.error(`Error sending night action prompt to ${player.displayName}:`, error);
      }
    }
  } catch (error) {
...
/**
 * Build action prompt message for a player
 ...
 */
function buildActionPrompt(player, role, targets, game) {
  let prompt = `🌙 **Daily Mafia - Night ${game.nightNumber}**\n\n`;
  prompt += `**Your Role:** ${role.emoji} ${role.name}\n`;
  prompt += `**Your Action:** ${role.actionType}\n\n`;

  // Add resource information if applicable
  if (player.bulletsRemaining !== undefined) {
    prompt += `Bullets remaining: **${player.bulletsRemaining}**\n`;
  }
  if (player.vestsRemaining !== undefined) {
    prompt += `Vests remaining: **${player.vestsRemaining}**\n`;
  }
  if (player.alertsRemaining !== undefined) {
    prompt += `Alerts remaining: **${player.alertsRemaining}**\n`;
  }
  if (player.jailsRemaining !== undefined) {
    prompt += `Jails remaining: **${player.jailsRemaining}**\n`;
  }
  if (player.seancesRemaining !== undefined) {
    prompt += `Seances remaining: **${player.seancesRemaining}**\n`;
  }

  prompt += `\n**Choose your target:**\n`;

  // List targets with numbers
  targets.forEach((target, index) => {
    prompt += `${index + 1}. ${target.displayName}\n`;
  });

  prompt += `\n**Or type:**\n`;
  if (role.actionType === 'alert') {
    prompt += `• \`alert\` - Go on alert\n`;
  }
  if (role.actionType === 'vest') {
    prompt += `• \`vest\` - Use protective vest\n`;
  }
  if (role.actionType === 'ignite') {
    prompt += `• \`ignite\` - Ignite doused players\n`;
  }
  prompt += `• \`skip\` - Skip your action\n\n`;

  prompt += `You can change your action as many times as you want until the phase ends.\n`;
  prompt += `Reply with a number, keyword, or \`skip\``;
...
async function handleActionSubmission(client, message) {
  try {
    ...
    if (game.phase !== 'night') {
      await message.reply('❌ It is not currently night phase. You cannot submit actions right now.');
      return;
    }
    ...
    if (!action) {
      await message.reply('❌ Invalid action. Please use a number, `skip`, or a valid keyword.');
      return;
    }
...
async function processNightActions(client, gameId) {
  try {
    ...
    console.log(`[Daily Mafia ${gameId}] Processing night ${game.nightNumber} actions`);

    const players = await gameState.getPlayers(gameId);
    const actions = await gameState.getActionsForNight(gameId, game.nightNumber);
