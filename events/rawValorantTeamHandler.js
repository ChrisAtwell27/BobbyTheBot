...
module.exports = (client) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (!message.content.includes(rawRoleMention) &&
        ...
        if (!member.roles.cache.has(RAW_VALORANT_ROLE_ID)) {
            if (isCommand) {
                message.reply('❌ You need the **RaW Valorant** role to create a RaW team!').catch(() => {});
            }
            return;
        }
            ...
            team.deleteTimer = setTimeout(async () => {
                ...
                if (currentTeam) {
                    ...
                    try {
                        ...
                        const msg = await ch.messages.fetch(currentTeam.messageId).catch(() => null);
    ...
    client.on('interactionCreate', async (interaction) => {
        try {
            ...
            if (interaction.isStringSelectMenu() && interaction.customId.startsWith('raw_valorant_selectleader_')) {
