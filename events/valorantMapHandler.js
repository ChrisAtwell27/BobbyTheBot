...
const VALORANT_MAPS = {
  ...
  Bind: {
    ...
    description:
      "Two sites. No middle. Gotta pick left or right. What's it going to be then?",
    type: "Standard",
    sites: 2,
    region: "Morocco",
  ...
  Haven: {
    ...
    description:
      "Beneath a forgotten monastery, three sites set the stage for large-scale engagements.",
    type: "Standard",
    sites: 3,
    region: "Bhutan",
  ...
  Abyss: {
    ...
    description:
      "A perilous battleground suspended high above an endless chasm.",
    type: "Standard",
    sites: 2,
    region: "Unknown",
  ...
  Corrode: {
    ...
    description:
      "A harsh industrial complex where metal meets corrosion in this gritty battleground.",
    type: "Standard",
    sites: 2,
    ...
    for (const fileName of allFiles) {
      ...
      if (SUPPORTED_FORMATS.includes(fileExt)) {
        for (const possibleName of possibleNames) {
          if (
            fileNameWithoutExt.includes(possibleName.toLowerCase()) ||
...
async function convertImageToStandard(inputPath, outputPath, mapName) {
  try {
    console.log(
      `Converting ${mapName}: ${path.basename(inputPath)} → ${path.basename(outputPath)}`
    );
...
function backupOriginalFile(originalPath, mapName) {
  try {
    ...
    const backupPath = path.join(
      BACKUP_DIR,
      `${mapName}_original_${originalFileName}`
    );

    if (!fs.existsSync(backupPath)) {
      fs.copyFileSync(originalPath, backupPath);
      console.log(
        `📁 Backed up original: ${originalFileName} → ${path.basename(backupPath)}`
      );
    }
  } catch (error) {
...
async function autoConvertMapImages() {
  ...
  for (const [mapName, mapData] of Object.entries(VALORANT_MAPS)) {
    ...
    if (foundImage) {
      ...
      const success = await convertImageToStandard(
        sourcePath,
        targetPath,
        mapName
      );
      ...
      if (success) {
        ...
        if (sourcePath !== targetPath) {
          try {
            ...
            console.warn(
              `⚠️ Could not remove old file ${foundImage}:`,
              error.message
            );
          }
        }
  ...
  if (convertedCount > 0) {
    console.log(
      "🎉 Image conversion completed! All maps now use standard format."
    );
  }
  ...
  if (notFoundCount > 0) {
    ...
    Object.keys(VALORANT_MAPS).forEach((mapName) => {
      if (!foundImages[mapName]) {
        console.log(
          `   • ${VALORANT_MAPS[mapName].fileName} (or any format with "${mapName}" in the name)`
        );
      }
    });
  }
...
function getRandomMap(excludeMaps = []) {
  const availableMaps = Object.keys(VALORANT_MAPS).filter(
    (map) => !excludeMaps.includes(map)
  );
...
async function createMapEmbed(mapName, isRandom = false) {
  ...
  const attachment = new AttachmentBuilder(mapCanvas.toBuffer(), {
    name: `${mapName.toLowerCase()}-display.png`,
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text: isRandom
        ? "Good luck on your selected map!"
        : "Use !randommap for a random selection",
    })
    .setTimestamp();
...
async function startMapVeto(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor("#ff4654")
    .setDescription(
      `**Step ${vetoSession.currentStep}/${vetoSession.maxSteps}:** Ban a map\n\nRemaining maps: **${vetoSession.remainingMaps.length}**`
    )
    .addFields({
...
module.exports = (client) => {
  if (!client._valorantMapHandlerInitialized) {
    console.log(
      "Valorant Map Handler - Auto-Converting Image Format loaded successfully!"
    );
    console.log(
      "Commands: !valorantmap, !randommap, !maplist, !convertmaps (admin)"
    );
    console.log(`Maps directory: ${MAPS_DIR}`);
    console.log(
      `Available maps: ${Object.keys(VALORANT_MAPS).length} (including new map: Corrode)`
    );
    ...
    client.on("messageCreate", async (message) => {
      ...
      if (
        !command.startsWith("!randommap") &&
        !command.startsWith("!valorantmap") &&
        !command.startsWith("!maplist") &&
        !command.startsWith("!maps") &&
        !command.startsWith("!convertmaps") &&
        !command.startsWith("!mapstatus")
      )
        return;
      ...
      if (command === "!randommap" || command === "!valorantmap") {
        try {
          ...
          message.reply(
            "❌ There was an error selecting a random map. Please try again."
          );
        }
      }
      ...
      if (command === "!maplist" || command === "!maps") {
        const embed = new EmbedBuilder()
          ...
          .setColor("#ff4654")
          .setDescription(
            `**${Object.keys(VALORANT_MAPS).length}** maps available for selection`
          )
          .addFields({
            name: "📋 Map Pool",
            value: Object.keys(VALORANT_MAPS)
              .map((map) => `• **${map}**`)
              .join("\n"),
            inline: false,
          })
      ...
      if (
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .addFields({
              name: "📊 Status",
              value: `✅ Correct format: ${conversionInfo.alreadyCorrect.length}\n❌ Missing: ${conversionInfo.missing.length}`,
              inline: false,
            })
          ...
          if (conversionInfo.missing.length > 0) {
            embed.addFields({
              name: "📝 Missing Images",
              value: conversionInfo.missing
                .map((map) => `• ${VALORANT_MAPS[map].fileName}`)
                .join("\n"),
              inline: false,
            });
          ...
          await loadingMessage.edit(
            "❌ Error occurred during conversion. Check console for details."
          );
        }
      }
      ...
      if (
        ...
        const embed = new EmbedBuilder()
          ...
          .setColor("#ff4654")
          .setDescription(
            `Checking ${Object.keys(VALORANT_MAPS).length} maps for proper format`
          )
          .addFields(
            {
              name: "✅ Correct Format",
              value: `${conversionInfo.alreadyCorrect.length} maps`,
              inline: true,
            },
            {
              name: "🔄 Need Conversion",
              value: `${conversionInfo.needsConversion.length} maps`,
              inline: true,
            },
            {
              name: "❌ Missing",
              value: `${conversionInfo.missing.length} maps`,
              inline: true,
            }
          )
          .setTimestamp();
        ...
        if (conversionInfo.needsConversion.length > 0) {
          embed.addFields({
            ...
            value: conversionInfo.needsConversion
              .map(
                (item) =>
                  `• ${item.foundImage} → Loading_Screen_${item.mapName}.jpg`
              )
              .join("\n"),
            inline: false,
          });
    ...
    client.on("interactionCreate", async (interaction) => {
      ...
      if (action === "veto" && parts.length > 3) {
        ...
        vetoSession.remainingMaps = vetoSession.remainingMaps.filter(
          (map) => map !== mapName
        );
        ...
        if (vetoSession.remainingMaps.length === 1) {
          ...
          const embed = new EmbedBuilder()
            ...
            .setColor("#ff4654")
            .setDescription(
              `**Step ${vetoSession.currentStep}/${vetoSession.maxSteps}:** ${vetoSession.remainingMaps.length > 1 ? "Ban a map" : "Final map selected!"}\n\nRemaining maps: **${vetoSession.remainingMaps.length}**`
            )
            .addFields(
              {
                name: "🗺️ Available Maps",
              ...
              {
                name: "🚫 Banned Maps",
                value: vetoSession.bannedMaps.join(", ") || "None",
                inline: false,
              }
            )
            .setFooter({ text: "Select maps to ban using the buttons below" })
          ...
          for (
            let i = 0;
            i < vetoSession.remainingMaps.length;
            i += mapsPerRow
          ) {
            const row = new ActionRowBuilder();
            const rowMaps = vetoSession.remainingMaps.slice(i, i + mapsPerRow);
