...
module.exports = (client) => {
  ...
  client.on("guildMemberUpdate", async (oldMember, newMember) => {
    const hadBoosterRole = oldMember.roles.cache.some(
      (role) => role.name === config.boosterRoleName
    );
    const hasBoosterRole = newMember.roles.cache.some(
      (role) => role.name === config.boosterRoleName
    );
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (
      !content.startsWith("!boosterrole") &&
      !content.startsWith("!color") &&
      !content.startsWith("!recolor") &&
      !content.startsWith("!rename") &&
      !content.startsWith("!deletecolor")
    )
      return;
    ...
    if (args[0] === "!boosterrole") {
      ...
      if (
        !member ||
        !member.roles.cache.some((role) => role.name === config.boosterRoleName)
      ) {
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Booster welcome message sent for ${targetMember.user.tag}`
        )
        .addFields(
          {
            name: "🎯 Target User",
          ...
          {
            name: "📍 Channel",
            value: `#${config.boosterChannelName}`,
            inline: true,
          },
      ...
      console.log(
        `🧪 Booster ${message.author.tag} triggered booster welcome for ${targetMember.user.tag}`
      );
      return;
    }
    ...
    if (args[0] === "!color") {
      ...
      if (!roleName || roleName.length > config.maxRoleNameLength) {
        return message.reply(
          `❌ Role name must be between 1 and ${config.maxRoleNameLength} characters.`
        );
      }

      if (!isValidHexColor(hexColor)) {
        return message.reply(
          "❌ Invalid hex color. Please use format: `#ff5733` or `#FF5733`"
        );
      }
    ...
    if (args[0] === "!rename") {
      ...
      if (newName.length > config.maxRoleNameLength) {
        return message.reply(
          `❌ Role name must be ${config.maxRoleNameLength} characters or less.`
        );
      }
    ...
    if (args[0] === "!colorhelp") {
      const helpEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "✏️ Rename Role",
            value: "`!rename <new name>`\nExample: `!rename My Awesome Role`",
            inline: false,
          },
          {
            name: "🗑️ Delete Role",
            value: "`!deletecolor`\nPermanently removes your custom role",
            inline: false,
          },
          {
            name: "🌈 Color Tips",
            value:
              "Use [color picker websites](https://htmlcolorcodes.com/) to find hex codes!\nFormat: `#ffffff` (with #)",
            inline: false,
          }
        )
  ...
  async function handleNewBooster(member) {
    try {
      const boosterChannel = member.guild.channels.cache.find(
        (channel) => channel.name === config.boosterChannelName
      );

      if (!boosterChannel || !boosterChannel.isTextBased()) {
        console.log(
          `Could not find booster channel: ${config.boosterChannelName}`
        );
        return;
      }

      // Check if user has a saved custom role
      const savedRole = getSavedBoosterRole(member.id, member.guild.id);
      ...
      if (savedRole) {
        ...
        const welcomeBackEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "🎨 Custom Role Restored",
              value: `Your previous role **${savedRole.name}** has been restored!`,
              inline: false,
            },
            {
              name: "✨ Want to Update?",
              value:
                "Use `!recolor <hex>` to change color or `!rename <name>` to change the name",
              inline: false,
            },
            {
              name: "❓ Need Help?",
              value: "Type `!colorhelp` for all available commands",
              inline: false,
            }
          )
        ...
        const welcomeEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "🎨 Create Your Custom Role",
              value: "You can now create a custom colored role just for you!",
              inline: false,
            },
            {
              name: "📝 How to Create",
              value:
                "`!color <role name> <hex color>`\nExample: `!color My Cool Role #ff5733`",
              inline: false,
            },
            {
              name: "🌈 Find Colors",
              value:
                "Visit [htmlcolorcodes.com](https://htmlcolorcodes.com/) to pick your perfect color!",
              inline: false,
            },
            {
              name: "❓ Need Help?",
  ...
  async function handleLostBooster(member) {
    try {
      ...
      if (customRole) {
        ...
        saveBoosterRole(member.id, member.guild.id, roleData);

        // Remove the custom role from the user
        await member.roles.remove(customRole, "No longer boosting server");

        console.log(
          `💔 Saved and removed custom role for ex-booster: ${member.user.tag}`
        );
  ...
  async function createCustomBoosterRole(message, member, roleName, hexColor) {
    try {
      ...
      if (existingRole) {
        return message.reply(
          "❌ You already have a custom role! Use `!recolor <hex>` to change color or `!rename <name>` to change the name."
        );
      }
      ...
      const colorRolesRole = member.guild.roles.cache.find(
        (role) => role.name === "-- COLOR ROLES --"
      );
      if (colorRolesRole) {
        await customRole.setPosition(colorRolesRole.position - 1);
      } else {
        // Fallback: put above booster role
        const boosterRole = member.guild.roles.cache.find(
          (role) => role.name === config.boosterRoleName
        );
        if (boosterRole) {
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Your custom role **${fullRoleName}** has been created and assigned!`
        )
        .addFields(
          ...
          {
            name: "💡 Pro Tip",
            value: "Your role will be saved even if you stop boosting!",
            inline: false,
          }
        )
        .setThumbnail(member.user.displayAvatarURL())
      ...
      console.log(
        `🎨 Created custom role for ${member.user.tag}: ${fullRoleName} (${hexColor})`
      );
    } catch (error) {
      console.error("Error creating custom booster role:", error);
      message.reply(
        "❌ Failed to create custom role. Please contact a moderator."
      );
    }
  }
  ...
  async function updateBoosterRoleColor(message, member, hexColor) {
    try {
      ...
      if (!customRole) {
        return message.reply(
          "❌ You don't have a custom role yet! Use `!color <name> <hex>` to create one."
        );
      }
      ...
      // Update saved data
      const roleData = getSavedBoosterRole(member.id, member.guild.id) || {};
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Your role color has been changed to **${hexColor.toUpperCase()}**`
        )
        .setThumbnail(member.user.displayAvatarURL())
        .setTimestamp();
      ...
      message.reply(
        "❌ Failed to update role color. Please contact a moderator."
      );
    }
  }
  ...
  async function updateBoosterRoleName(message, member, newName) {
    try {
      ...
      console.log(
        `✏️ Updated role name for ${member.user.tag}: ${fullRoleName}`
      );
    } catch (error) {
      console.error("Error updating booster role name:", error);
      message.reply(
        "❌ Failed to update role name. Please contact a moderator."
      );
    }
  }
  ...
  async function deleteCustomBoosterRole(message, member) {
    try {
      ...
      removeSavedBoosterRole(member.id, member.guild.id);
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Your custom role **${roleName}** has been permanently deleted.`
        )
        .addFields({
          name: "💡 Want a new one?",
          value: "Use `!color <name> <hex>` to create a new custom role!",
          inline: false,
        })
        .setThumbnail(member.user.displayAvatarURL())
        .setTimestamp();
      ...
      message.reply(
        "❌ Failed to delete custom role. Please contact a moderator."
      );
    }
  }
  ...
  async function restoreBoosterRole(member, savedRole) {
    try {
      ...
      if (!existingRole) {
        ...
        saveBoosterRole(member.id, member.guild.id, savedRole);
      }
      ...
      console.log(
        `🔄 Restored custom role for ${member.user.tag}: ${savedRole.name}`
      );
    } catch (error) {
  ...
  function saveBoosterRole(userId, guildId, roleData) {
  ...
  function getSavedBoosterRole(userId, guildId) {
    if (!fs.existsSync(boosterRolesFilePath)) {
      return null;
    }
    ...
    // Try precise match first (userId_guildId)
    // If legacy data exists (just userId), we might want to migrate it or just ignore.
    // For now, let's assume new format only or precise match.
    const recordKey = `${userId}_${guildId}`;
    const userRecord = data
      .split("\n")
      .find((line) => line.startsWith(recordKey + ":"));

    if (userRecord) {
      const [key, name, color, roleId] = userRecord.split(":");
      return {
        name: name || "",
        color: color || "#9932CC",
        roleId: roleId || null,
  ...
  function removeSavedBoosterRole(userId, guildId) {
    if (!fs.existsSync(boosterRolesFilePath)) {
      return;
    }
    ...
    const lines = data
      .split("\n")
      .filter(
        (line) => line.trim() !== "" && !line.startsWith(recordKey + ":")
      );
