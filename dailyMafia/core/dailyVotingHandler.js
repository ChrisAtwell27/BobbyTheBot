...
async function handleVoteCommand(client, message, args) {
  try {
    ...
    if (!targetId) {
      await message.reply('❌ Invalid target. Use `!vote @player` or `!vote skip`.');
      return;
    }
...
async function handleUnvoteCommand(client, message) {
  try {
    ...
    // Delete vote
    const success = await gameState.deleteVote(game.gameId, game.dayNumber, message.author.id);
...
async function submitVote(client, gameId, voterId, targetId) {
  try {
    ...
    const targetName = targetId === 'skip' ? 'skip' :
      (await gameState.getPlayer(gameId, targetId))?.displayName || 'Unknown';
...
async function tallyVotes(client, gameId) {
  try {
    ...
    console.log(`[Daily Mafia ${gameId}] Tallying votes for Day ${game.dayNumber}`);
    ...
    if (!mostVoted || isTie || mostVoted === 'skip') {
      // No elimination
      const message = `🗳️ **[Game ${gameId}] Voting Results**\n\n` +
      ...
      if (target) {
        ...
        await gameState.createEvent({
          ...
          phaseNumber: game.dayNumber,
          eventType: 'death',
          description: `${target.displayName} was lynched (${target.role})`,
        ...
        console.log(`[Daily Mafia ${gameId}] ${target.displayName} was lynched`);
      }
    }
