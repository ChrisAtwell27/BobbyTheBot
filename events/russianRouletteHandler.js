...
async function createDeathResultVisualization(victim, survivors, totalPot, winnings) {
    ...
    try {
        ...
        const avatarURL = victim.avatarURL || 
...
module.exports = (client) => {
            ...
            if (userBalance <= 0) {
                return message.channel.send({
                    embeds: [new EmbedBuilder()
                        ...
                        .setDescription('You need at least B1 to play Russian Roulette!\nYou cannot risk what you do not have.')
                        .setTimestamp()]
                });
            }
            ...
            if (existingLobby) {
                return message.channel.send({
                    embeds: [new EmbedBuilder()
                        ...
                        .setDescription('There is already a Russian Roulette lobby in this channel!\nJoin the existing game or wait for it to finish.')
                        .setTimestamp()]
                });
            }
    ...
    async function startCountdown(interaction, lobby) {
        ...
        for (let i = COUNTDOWN_TIME; i > 0; i--) {
            ...
            try {
                ...
                const attachment = new AttachmentBuilder(countdownImageBuffer, { name: 'countdown.png' });
                
                const countdownEmbed = new EmbedBuilder()
                    .setColor(i <= 3 ? '#ff0000' : '#ff6b6b')
                    .setTitle('🔫 RUSSIAN ROULETTE - COUNTDOWN')
                    .setDescription(i <= 3 ? '**💀 DEATH APPROACHES 💀**' : 'Prepare for your fate...')
    ...
    async function executeRussianRoulette(interaction, lobby) {
        ...
        const totalPot = lobby.players.reduce((total, player) => total + player.balance, 0);
        ...
        try {
            ...
            const attachment = new AttachmentBuilder(resultImageBuffer, { name: 'death-result.png' });

            const resultEmbed = new EmbedBuilder()
                .setColor('#8b0000')
                .setTitle('💀 THE CHAMBER WAS NOT EMPTY 💀')
                .setDescription(`**${victim.displayName || victim.username}** has been claimed by fate!`)
    ...
    async function createLobbyEmbed(lobby) {
        ...
        const attachment = new AttachmentBuilder(tableImageBuffer, { name: 'roulette-table.png' });
        
        const embed = new EmbedBuilder()
            .setColor('#8b0000')
            .setTitle('🔫 RUSSIAN ROULETTE LOBBY')
            .setDescription('**⚠️ WARNING: ONE PLAYER WILL LOSE EVERYTHING! ⚠️**\n\nThe eliminated player will lose their ENTIRE balance.\nSurvivors will split the victim\'s money equally.')
            ...
            .setFooter({ 
                text: lobby.players.length >= MIN_PLAYERS ? 
