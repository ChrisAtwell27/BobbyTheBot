...
const { sendNightActionPrompts, handleActionSubmission } = require('../dailyMafia/core/dailyActionHandler');
const {
  handleVoteCommand,
  handleUnvoteCommand,
  handleVotesCommand,
...
async function handleMessage(client, message) {
  try {
    ...
    switch (command) {
      ...
      default:
        await message.reply(`❌ Unknown command. Use \`${PREFIX} help\` for a list of commands.`);
    }
...
async function handleJoinCommand(client, message) {
  try {
    ...
    const game = games.find(g => g.channelId === message.channelId && g.status === 'pending');

    if (!game) {
      await message.reply('❌ No pending Daily Mafia game in this channel. Use `!dailymafia start` to create one.');
      return;
    }

    // Check if player already in game
    const existingPlayer = await gameState.getPlayer(game.gameId, message.author.id);
    if (existingPlayer) {
    ...
    const playerActiveGame = await gameState.getPlayerActiveGame(message.author.id);
    if (playerActiveGame) {
...
async function handleLeaveCommand(client, message) {
  try {
    ...
    await message.reply('❌ Cannot leave game after joining. Ask the organizer to cancel the game.');
...
async function handleJoinButton(client, interaction, gameId) {
  ...
  if (!game || game.status !== 'pending') {
    await interaction.reply({ content: '❌ This game is no longer accepting players.', ephemeral: true });
    return;
  }
...
async function handleLeaveButton(client, interaction, gameId) {
  await interaction.reply({
    content: '❌ Cannot leave after joining. Ask the organizer to cancel the game.',
    ephemeral: true,
  });
}
...
async function handleStartButton(client, interaction, gameId) {
  ...
  if (game.organizerId !== interaction.user.id) {
    await interaction.reply({ content: '❌ Only the organizer can start the game.', ephemeral: true });
    return;
  }

  const players = await gameState.getPlayers(gameId);
  if (players.length < 8) {
    await interaction.reply({ content: `❌ Need at least 8 players to start (currently ${players.length}).`, ephemeral: true });
    return;
  }
...
async function handleCancelButton(client, interaction, gameId) {
  ...
  if (game.organizerId !== interaction.user.id) {
    await interaction.reply({ content: '❌ Only the organizer can cancel.', ephemeral: true });
    return;
  }
