...
module.exports = (client) => {
  client.on("messageCreate", async (message) => {
    ...
    const isEconomyCommand =
      ...
      content.startsWith("!baltop") ||
      content.startsWith("!award") ||
      content.startsWith("!spend") ||
      content.startsWith("!pay") ||
      content.startsWith("!beg") ||
      content.startsWith("!economy") ||
      content.startsWith("!clearhoney");
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          },
          {
            ...
            value:
              balance > 1000 ? "🌟 **Premium Hive**" : "🐝 **Standard Hive**",
            inline: true,
          },
      ...
      if (balance >= amount) {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: "📊 Savings Rate",
              value: `${((newBalance / oldBalance) * 100).toFixed(1)}%`,
              inline: true,
            }
          )
        ...
        const insufficientFunds = await createInsufficientFundsCard(
          message.author,
          balance,
          amount
        );
        const attachment = new AttachmentBuilder(insufficientFunds.toBuffer(), {
          name: "insufficient-funds.png",
        });
    ...
    if (args[0] === "!awardall" && args[1]) {
      ...
      try {
        ...
        const massAwardImage = await createMassAwardCard(
          amount,
          membersAwarded,
          message.author
        );
        const attachment = new AttachmentBuilder(massAwardImage.toBuffer(), {
          name: "mass-award.png",
        });
        ...
        return message.channel.send(
          "❌ Failed to fetch all server members. Please try again."
        );
      }
    }
    ...
    if (args[0] === "!pay" && args[1] && args[2]) {
      ...
      if (isNaN(amount) || amount <= 0) {
        return message.channel.send(
          "❌ Invalid amount specified. Amount must be a positive number."
        );
      }
      ...
      const attachment = new AttachmentBuilder(paymentReceipt.toBuffer(), {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor("#00ff00")
        .setDescription(
          `**${message.author.username}** paid **${mentionedUser.username}**`
        )
    ...
    if (args[0] === "!beg") {
      ...
      const attachment = new AttachmentBuilder(tipJarImage.toBuffer(), {
        name: "tip-jar.png",
      });
    ...
    if (args[0].toLowerCase() === "!clearhoney") {
      ...
      // Estimate count (optional, or just say 'all users')
      const userCount = "ALL";
      ...
      const confirmEmbed = new EmbedBuilder()
        ...
        .setFooter({
          text: 'Click "Confirm Reset" to proceed or "Cancel" to abort',
        })
        .setTimestamp();
      ...
      collector.on("collect", async (interaction) => {
        if (interaction.customId === `confirm_reset_${message.author.id}`) {
          ...
          await interaction.update({
            embeds: [
              new EmbedBuilder()
                ...
                .setColor("#ffa500")
                .setDescription(
                  "Please wait while all balances are being reset..."
                ),
            ],
            components: [],
          });
          ...
          try {
            ...
            // Reset all balances to 5000 using Convex mutation
            const result = await client.mutation(api.users.resetAllBalances, {
              amount: 5000,
            });

            console.log(
              "Honey reset complete. Modified:",
            ...
            await interaction.editReply({
              embeds: [
                new EmbedBuilder()
                  ...
                  .setColor("#ff0000")
                  .setDescription(
                    "An error occurred while resetting honey balances. Please try again later or check the console for details."
                  ),
              ],
              components: [],
            });
          ...
          interaction.customId === `cancel_reset_${message.author.id}`
        ) {
          // User cancelled
          await interaction.update({
            embeds: [
              new EmbedBuilder()
                ...
                .setColor("#95a5a6")
                .setDescription(
                  "The honey balance reset has been cancelled. No changes were made."
                )
          ...
          confirmMessage.edit({
            embeds: [
              new EmbedBuilder()
                ...
                .setColor("#95a5a6")
                .setDescription(
                  "The confirmation request has expired. No changes were made."
                )
                .setFooter({
                  text: "Run !clearhoney again if you still want to reset balances",
                }),
            ],
            components: [],
          });
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId.startsWith("donate_")) {
      ...
      const donationReceipt = await createDonationReceipt(
        donor,
        beggar.user,
        actualDonation,
        donorOldBalance,
      ...
      const attachment = new AttachmentBuilder(donationReceipt.toBuffer(), {
        name: "donation-receipt.png",
      });
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor("#00ff00")
        .setDescription(
          `**${donor.username}** donated to **${beggar.user.username}**!`
        )
      ...
      try {
        ...
        const updatedTipJar = await createTipJarCard(
          beggar.user,
          beggarNewBalance,
          donor.username,
          actualDonation
        ...
        const updatedAttachment = new AttachmentBuilder(
          updatedTipJar.toBuffer(),
          { name: "tip-jar-updated.png" }
        );
        ...
        const updatedEmbed = new EmbedBuilder()
          ...
          .setColor("#ff9500")
          .setDescription(
            `**${beggar.user.username}** is asking for your kindness!`
          )
    ...
    try {
      const welcomeEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "📚 Learn Commands",
            value: "Use `!help` to explore all features",
            inline: true,
          },
          {
            name: "🎮 Start Playing",
            value:
              "Try `!gamble`, `!blackjack`, or `!rps` to have fun and earn more!",
            inline: false,
          },
          {
            name: "💡 Daily Earnings",
            value: "Use `!beg` daily for free Honey",
            inline: false,
          }
        )
      ...
      console.log(
        `[ECONOMY] Sent welcome message to ${member.user.username} with 500 honey bonus`
      );
    } catch (error) {
      // User has DMs disabled - that's okay, they still get the bonus
      console.log(
        `[ECONOMY] Could not send welcome DM to ${member.user.username} (DMs disabled), but they received their 500 honey bonus`
      );
    }
  });
  ...
  async function createInsufficientFundsCard(user, balance, attemptedAmount) {
    ...
    ctx.fillText(
      `Needed: 🍯${(attemptedAmount - balance).toLocaleString()}`,
      200,
      180
    );
  ...
  async function createMassAwardCard(amount, membersCount, admin) {
    ...
    ctx.font = "18px Arial";
    ctx.fillText(
      `Total distributed: 🍯${(amount * membersCount).toLocaleString()}`,
      250,
      180
    );
  ...
  async function getEconomyStats(guild) {
    ...
    return {
      ...
      averageBalance:
        balances.length > 0 ? Math.floor(totalEconomy / balances.length) : 0,
  ...
  // Create tip jar visualization
  async function createTipJarCard(
    user,
    balance,
    lastDonor = null,
    lastAmount = null
  ...
  // Create donation receipt
  async function createDonationReceipt(
    donor,
    beggar,
    amount,
    donorOldBalance,
  ...
  async function createPaymentReceipt(
    ...
    ctx.fillText(
      `  Before: 🍯${recipientOldBalance.toLocaleString()}`,
      30,
      335
    );
