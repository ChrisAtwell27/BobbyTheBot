...
async function createChallenge(message, args, type, options) {
  if (
    ...
    return message.channel.send(
      invalidUsageMessage(type, `!${type} [amount]`, `!${type} 100`)
    );
  }
  ...
  const processingMsg = await message.channel.send(
    processingMessage("we create your challenge")
  );
  ...
  await updateBalance(guildId, userId, -betAmount); // Lock the bet
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      options.description.replace("{user}", message.author.username)
    )
    .addFields(
      {
        name: "💰 Pot",
      ...
      { name: "✅ To Accept", value: `Click the button below!`, inline: true },
      {
        name: "⏳ Expires",
        value:
          "<t:" + Math.floor((Date.now() + CHALLENGE_TIMEOUT) / 1000) + ":R>",
        inline: true,
      }
    );
  ...
  embed
    .setFooter({ text: options.footer || "Click to accept!" })
    .setTimestamp();
  ...
  challenge.expiryTimer = setTimeout(async () => {
    if (challenges.has(challengeId)) {
      ...
      await updateBalance(guildId, userId, betAmount); // Refund

      // Delete from database
      try {
        const client = getConvexClient();
        await client.mutation(api.challenges.deleteChallenge, { challengeId });
      } catch (dbError) {
        console.error(
          "[GAMBLING] Error deleting expired challenge from database:",
          dbError
        );
      }
      ...
      try {
        ...
        if (channel) {
          ...
          await channel.send(
            `⚠️ ${options.title} by **${authorUsername}** has expired and been refunded.`
          );
        }
      } catch (error) {
...
async function handleChallengeAccept(interaction) {
  ...
  if (!challenge) {
    ...
    try {
      const client = getConvexClient();
      const dbChallenge = await client.query(api.challenges.getChallenge, {
        challengeId,
      });
      if (dbChallenge) {
        challenge = {
          ...
          timestamp: dbChallenge.createdAt,
        };
        challenges.set(challengeId, challenge);
      ...
      console.error(
        "[GAMBLING] Error loading challenge from database:",
        dbError
      );
    }
  }
  ...
  if (!challenge) {
    return interaction
      .reply({
        content:
          "⚠️ This challenge is no longer active. It may have expired.\n\n💡 **Tip:** Ask the challenger to create a new challenge!",
        ephemeral: true,
      })
      .catch((err) => {
        if (err.code !== 10062 && err.code !== 40060) {
          console.error(
            "Error replying to expired challenge interaction:",
            err
          );
        }
      });
  ...
  if (challenge.creator === userId) {
    return interaction.reply({
      content: "⚠️ You can't accept your own challenge!",
      ephemeral: true,
    });
  }
  ...
  try {
    ...
    console.error(
      "[GAMBLING] Error deleting challenge from database:",
      dbError
    );
  }
  ...
  await interaction.reply({
    content: `✅ Challenge accepted! Starting **${challenge.type.toUpperCase()}** game...`,
    ephemeral: true,
  });
...
async function handleGameMove(interaction, client) {
  ...
  if (!activeGames.has(gameId)) {
    ...
    if (!foundGame) {
      return interaction.reply({
        content:
          "⚠️ No active RPS game found for you. The game may have expired.",
        ephemeral: true,
      });
    }
    ...
    if (game.moves[userId]) {
      return interaction.reply({
        content: "⚠️ You've already made your move!",
        ephemeral: true,
      });
    }
    ...
    await interaction.reply({
      content: `✅ Your move (${move}) has been recorded! Waiting for your opponent...`,
      ephemeral: true,
    });
  ...
  if (userId !== game.player1 && userId !== game.player2) {
    return interaction.reply({
      content: "⚠️ You're not part of this game!",
      ephemeral: true,
    });
  }
  ...
  client.on("messageCreate", async (message) => {
      ...
      if (command === "!play" && args.length >= 3) {
        ...
        if (game.type === "numberduel") {
          ...
          if (isNaN(guess) || guess < 1 || guess > 100) {
            return message.reply(
              "Invalid guess! Please enter a number between 1-100"
            );
          }
        ...
        const helpEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "🎮 Number Duel",
              value: "`!play [game_id] [number]`\nSubmit your guess (1-100)",
              inline: false,
            },
            { name: "📋 Example", value: "`!play game_456 42`", inline: false },
            {
              name: "💡 Tip",
              value: "All other games use buttons in the main chat now!",
              inline: false,
            }
          )
    ...
    if (command === "!gamble") {
      ...
      const gameList = new EmbedBuilder()
        ...
        .setColor("#ffd700")
        .setDescription(
          `**Welcome to the Honey Casino!** 🎲\nChoose your game and test your luck!\n\n**Your Server Tier:** ${userTier.toUpperCase()} ${userTier === TIERS.FREE ? "(Free)" : userTier === TIERS.PLUS ? "⭐" : "💎"}`
        )
        .addFields(formatGamblingCommandsForEmbed(userTier))
        .addFields(
          {
            name: "💰 Your Stats",
          ...
          },
          {
            name: "🎯 Payouts",
            value:
              "**Solo:** Various multipliers\n**PvP:** Winner takes all!\n**House Cut:** 5% of pot",
            inline: true,
          }
        )
        .setFooter({
          text: "🍀 Good luck and gamble responsibly! | Upgrade for more games!",
        })
        .setTimestamp();
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    try {
      ...
      if (error.code === 10062 || error.code === 40060) {
        ...
        console.log(
          "Interaction expired or unknown - user clicked an old button"
        );
      } else {
...
async function startRPSGame(interaction, gameId, game) {
  ...
  const gameEmbed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "ðŸ”’ Privacy",
        value: "Your moves will be hidden until both choose!",
        inline: false,
      },
      { name: "ðŸ†” Game ID", value: `\`${gameId}\``, inline: true }
    )
    .setFooter({
      text: "Rock 🪨 beats Scissors ✂️ | Paper 📄 beats Rock 🪨 | Scissors ✂️ beats Paper 📄",
    })
    .setTimestamp();
...
async function startHigherCardGame(interaction, gameId, game) {
  ...
  const player1Card = {
    rank: ranks[card1Index],
    suit: suit1,
    value: values[card1Index],
  };
  const player2Card = {
    rank: ranks[card2Index],
    suit: suit2,
    value: values[card2Index],
  };
  ...
  if (player1Card.value > player2Card.value) {
    ...
    await updateBalance(game.guildId, game.player1, game.amount);
    await updateBalance(game.guildId, game.player2, game.amount);
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: `${game.player1Name}'s Card`,
          value: `${player1Card.rank}${player1Card.suit} (${player1Card.value})`,
          inline: true,
        },
        {
          name: `${game.player2Name}'s Card`,
          value: `${player2Card.rank}${player2Card.suit} (${player2Card.value})`,
          inline: true,
        },
  ...
  await updateBalance(game.guildId, winner, winnings);
  await updateHouse(houseCut);
  activeGames.delete(gameId);

  const winnerName =
    winner === game.player1 ? game.player1Name : game.player2Name;
  const embed = new EmbedBuilder()
...
async function startQuickDrawGame(interaction, gameId, game) {
  ...
  const randomWord =
    quickDrawWords[Math.floor(Math.random() * quickDrawWords.length)];
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "🎯 Get Ready!", value: "Wait for the signal...", inline: false },
      {
        ...
        value: "When prompted, type the **exact word** shown to win!",
        inline: false,
      }
    )
    .setTimestamp();
...
async function startNumberDuelGame(interaction, gameId, game) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "📋 Rules",
        value: "Closest to the secret number wins!",
        inline: false,
      },
      {
        name: "📩 Important",
        value: "Both players must send their guesses via **Direct Message**!",
        inline: false,
      }
    )
...
async function resolveRPSGame(client, gameId, game) {
    ...
    channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setColor("#e74c3c")
          .setDescription(
            "Game could not be resolved due to invalid or missing moves."
          ),
      ],
    });
    activeGames.delete(gameId);
  ...
  if (result === "tie") {
      ...
      .addFields(
        {
          ...
          value: `${moveEmojis[move1]} ${move1}`,
          inline: true,
        },
        {
          name: game.player2Name,
          value: `${moveEmojis[move2]} ${move2}`,
          inline: true,
        },
        { name: "💰 Result", value: "Both players refunded!", inline: false }
      );
    ...
    const winnerName =
      result === "player1" ? game.player1Name : game.player2Name;
    const houseCut = Math.floor(game.amount * 2 * 0.05);
    const winnings = game.amount * 2 - houseCut;
  ...
  if (diff1 === diff2) {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: game.player1Name,
          value: `${guess1} (off by ${diff1})`,
          inline: true,
        },
        {
          name: game.player2Name,
          value: `${guess2} (off by ${diff2})`,
          inline: true,
        },
        { name: "💰 Result", value: "Both players refunded!", inline: false }
      );
...
async function listChallenges(message) {
  ...
  if (currentChallenges.length === 0) {
    return message.channel.send(
      "No active challenges right now! Create one with !rps, !highercard, !quickdraw, or !numberduel"
    );
  }
...
async function createCoinFlipAnimation(
  result,
  playerName,
  betAmount,
  winnings
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    isWin ? `YOU WIN 🍯${winnings}!` : `YOU LOSE 🍯${Math.abs(winnings)}!`,
    200,
    250
  );
...
async function createRouletteWheel(
  ...
  ctx.font = "14px Arial";
  ctx.fillText(
    `${playerName} bet 🍯${betAmount} on ${userChoice.toUpperCase()}`,
    250,
    50
  );
  ...
  for (let i = 0; i < 37; i++) {
    ...
    // Segment
    ctx.fillStyle =
      numbers[i] === 0 ? "#00ff00" : isRed ? "#ff0000" : "#000000";
    if (isWinning) {
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    `Winning: ${chosenColor.toUpperCase()} ${chosenNumber}`,
    250,
    320
  );
...
async function createDiceRoll(
  ...
  ctx.font = "16px Arial";
  ctx.fillText(
    `${playerName} guessed ${userGuess} - Bet: 🍯${betAmount}`,
    200,
    50
  );
...
async function playFlipGame(message, args) {
  if (
    ...
    return message.channel.send(
      "Incorrect usage! Correct syntax: !flip [positive amount]"
    );
  }
  ...
  await updateBalance(guildId, userId, netGain);
  await updateHouse(-netGain);

  // Create visual
  const coinCanvas = await createCoinFlipAnimation(
    result,
    message.author.username,
    betAmount,
    netGain
  );
  const attachment = new AttachmentBuilder(coinCanvas.toBuffer(), {
    name: "coin-flip.png",
  });
...
async function playRouletteGame(message, args) {
  if (
    args.length !== 3 ||
    isNaN(parseInt(args[1], 10)) ||
    (!["red", "black"].includes(args[2].toLowerCase()) &&
      (isNaN(parseInt(args[2], 10)) ||
        parseInt(args[2], 10) < 0 ||
        parseInt(args[2], 10) > 36)) ||
    parseInt(args[1], 10) <= 0
  ) {
    return message.channel.send(
      "Incorrect usage! Correct syntax: !roulette [positive amount] [red/black/0-36]"
    );
  }
  ...
  const chosenColor =
    chosenNumber === 0
      ? "green"
      : redNumbers.includes(chosenNumber)
        ? "red"
        : "black";
  const userChoice = args[2].toLowerCase();

  let winnings = 0;
  if (userChoice === chosenColor && chosenNumber !== 0)
  ...
  // Create visual
  const rouletteCanvas = await createRouletteWheel(
    chosenNumber,
    chosenColor,
    userChoice,
    message.author.username,
  ...
  const attachment = new AttachmentBuilder(rouletteCanvas.toBuffer(), {
    name: "roulette-wheel.png",
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: "🎯 Result",
        value: `**${chosenColor.toUpperCase()} ${chosenNumber}**`,
        inline: true,
      },
      {
        name: "🎲 Your Bet",
        value: `**${userChoice.toUpperCase()}**`,
        inline: true,
      },
      {
        name: "💰 Outcome",
    ...
    .addFields(
      ...
      {
        name: "🎰 Payout Rate",
        value: userChoice === String(chosenNumber) ? "36:1" : "2:1",
        inline: true,
      }
    )
    .setFooter({
      text:
        netGain > 0
          ? "🎉 The wheel of fortune favors you!"
...
async function playDiceGame(message, args) {
  if (
    ...
    parseInt(args[2], 10) > 6 ||
    parseInt(args[1], 10) <= 0
  ) {
    return message.channel.send(
      "Incorrect usage! Correct syntax: !dice [positive amount] [guess (1-6)]"
    );
  }
  ...
  // Create visual
  const diceCanvas = await createDiceRoll(
    diceRoll,
    userGuess,
    message.author.username,
    betAmount,
  ...
  const attachment = new AttachmentBuilder(diceCanvas.toBuffer(), {
    name: "dice-roll.png",
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "🎰 Match?",
        value: userGuess === diceRoll ? "✅ **WINNER!**" : "❌ **Miss**",
        inline: true,
      }
    )
    .addFields(
    ...
    .setFooter({
      text:
        netGain > 0
          ? "🎉 Perfect prediction! You nailed it!"
